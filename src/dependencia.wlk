...
class Dependencia {
	...
	method capacidadTotalEnColor(color){return rodados.map({rodado=> rodado.color() == color}).sum({r=>r.capacidad()})}
	method colorDelRodadoMasRapido(){return rodados.max({rodado => rodado.velocidad()}).map({rodado=>rodado.color()})}
